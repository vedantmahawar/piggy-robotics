export default function Home() {
  return (
    <div className="w-full max-w-6xl mx-auto px-8 xl:px-0 py-50 sm:pt-60 sm:pb-40 text-balance">
      <h1 className="tracking-tight text-4xl sm:text-4xl font-semibold mb-2">
        The Team
      </h1>
      <p className="text-lg mb-4">
        We're a team of{" "}
        <span className="font-semibold">
          sixteen elite hardware & software engineers
        </span>{" "}
        from <span className="font-semibold">Oxford University</span> & top
        global robotics competitions (yes, we attended robotics competitions
        like the{" "}
        <span className="font-semibold">
          IEEE/RSJ International Conference on Intelligent Robots and Systems
        </span>{" "}
        and hired the most cracked engineers).
        <br />
        <br />
        We have a <span className="italic">deep</span> understanding of{" "}
        <span className="font-semibold">
          human anatomy, biomechanics, AI motion,
        </span>{" "}
        and <span className="font-semibold">manufacturing.</span>
        <br />
        <br />
        <span className="font-semibold">Richard</span>, one of our cofounders,
        built AI systems for{" "}
        <span className="font-semibold">personalized cancer vaccines</span>{" "}
        while studying <span className="font-semibold">Medicine</span> at Oxford
        University.
        <br />
        <br />
        <span className="font-semibold">Chenny</span>, our other cofounder,
        trained AI models to read{" "}
        <span className="font-semibold">human motion</span> while doing her{" "}
        <span className="font-semibold">PhD in Computer Science</span> at Oxford
        University. She previously worked on AI at{" "}
        <span className="font-semibold">Tencent</span>.
        <br />
        <br />
        Together, they spent a year inside factories across the world deploying
        humanoid robots in real production environments.
        <br />
        <br />
        <span className="font-bold bg-yellow-200">Speed</span> is the culture at
        Piggy Robotics. Everyone works, sleeps, and lives in our six-story
        hacker house, where we prototype daily.
        <br />
        <br />
        Given our technical capacity, domain-specific knowledge, and culture, we
        believe we can match incumbents & their humanoids within a year.
      </p>
    </div>
  );
}
